...
const url = "http://127.0.0.1:4174/articles/wechat-automation-tools/publish/wechat-standard.html";
const outDir = "C:\\html\\articles\\wechat-automation-tools\\publish";
const port = 9367;
    ...
    const metricsResult = await send("Runtime.evaluate", {
      ...
      expression: `(() => {
        ...
        return {
          ...
          highlightBlocks: sections.filter(el => {
            const s = el.getAttribute('style') || '';
            return s.includes('#eef5f3') || s.includes('#fff7e8');
          }).length,
...
const profile = await mkdtemp(path.join(os.tmpdir(), "wechat-publish-verify-"));
